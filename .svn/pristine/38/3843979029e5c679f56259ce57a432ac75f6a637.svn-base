...
- (void)viewDidLoad {
    self.viewModel = [[MessagesViewModel alloc]initWithViewController:self];
    [super viewDidLoad];
}
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    NSLog(@"%@",self.webView);
    [self.webView loadRequestWithUrlString:xianHuaWangURL];
    self.navigationController.navigationBarHidden = NO;
}
- (void)dealloc {
    [self.webView.configuration.userContentController
     removeScriptMessageHandlerForName:MessageHandlerMessages];
}
...
- (void)webViewLoadRequestWithUrlString:(NSString *)urlStr
    ...
    [super webViewLoadRequestWithUrlString:messagesURL
                          endConfiguration:config
                               endDelegate:self.viewModel
                    endtMessageHandlerName:MessageHandlerMessages];
}
