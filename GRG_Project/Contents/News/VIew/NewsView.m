...
- (void)viewDidLoad {
    self.viewModel = [[NewsViewModel alloc]initWithViewController:self];
    [super viewDidLoad];
}
...
- (void)dealloc {
    [self.webView.configuration.userContentController
     removeScriptMessageHandlerForName:MessageHandlerNews];
}
...
- (void)webViewLoadRequestWithUrlString:(NSString *)urlStr
    ...
    [super webViewLoadRequestWithUrlString:newsURL
                          endConfiguration:config
                               endDelegate:self.viewModel
                    endtMessageHandlerName:MessageHandlerNews];
}
